...
class penjualan
{
	...
	{
		$this->con=new mysqli("localhost","root","","2");
	}
	function __destruct()
	{
		$this->con->close();
	...
	function tambah_pelayan($nama_pelayan,$password){
		$nama_pelayan=str_replace(" ", "", $nama_pelayan);
		$query=$this->con->query("select * from  user where username='$nama_pelayan' and status='2'");
		if ($query->num_rows > 0) {
			$this->alert("Username  untuk pelayan  sudah ada.");
			$this->go_back();
		}
		else{
			$password=sha1($password);
			$query2=$this->con->query("insert into user set username='$nama_pelayan',password='$password',status='2'");
			if ($query2 ===  TRUE) {
				$this->alert("Data pelayan berhasil dismpan");
				$this->redirect("users.php");
			}
			else{
				$this->alert("Pelayan Gagal Ditambahkan");
				$this->redirect("users.php");
			}
		}
	}
	...
	function tampil_laporan(){
		$query=$this->con->query("select transaksi.id_transaksi,transaksi.tgl_transaksi,transaksi.no_invoice,transaksi.total_bayar,transaksi.nomor_meja,user.username from transaksi inner join user on transaksi.kode_kasir=user.id order by transaksi.id_transaksi desc");
		$no=1;
	...
	function filter_tampil_laporan($tanggal,$aksi){
		if ($aksi==1) {
			...
			$query=$this->con->query("select transaksi.id_transaksi,transaksi.tgl_transaksi,transaksi.no_invoice,transaksi.total_bayar,transaksi.nomor_meja,user.username from transaksi inner join user on transaksi.kode_kasir=user.id where transaksi.tgl_transaksi like '%$tanggal%' order by transaksi.id_transaksi desc");
		}else{
			$split1=explode('-',$tanggal);
