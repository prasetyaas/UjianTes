...
if (isset($_GET['action'])) {
	...
	if ($action=="tambah_pelayan") {
		$root->tambah_pelayan($_POST['nama_pelayan'],$_POST['password']);
	}
	...
	if ($action=="selesai_transaksi") {
		...
		$trx=date("d")."/AF/".$_SESSION['id']."/".date("y/h/i/s");
			...
			$query=$root->con->query("insert into transaksi set kode_kasir='$_SESSION[id]',total_bayar='$_POST[total_bayar]',no_invoice='$trx',nomor_meja='$_POST[nomor_meja]'");

		$trx2=date("d")."/AF/".$_SESSION['status']."/".date("y");
